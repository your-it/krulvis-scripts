...
@ScriptManifest(
	name = "krul Runecrafter", version = "1.0.6",
...
class Runecrafter : ATScript() {
	...
	private val dreamMentorBankTile = Tile(2099, 3920, 0)
	fun getBank(): InteractableEntity {
		val eniola = Npcs.stream().name("Eniola").first()
		val jack = Npcs.stream().name("""'Bird's-eye' Jack""").isNotEmpty()
		...
		return Objects.stream(25, GameObject.Type.INTERACTIVE).name("Bank booth", "Bank chest").action("Use", "Bank")
			.filtered { it.tile != dreamMentorBankTile || jack }
			.nearest().first()
	}
